...
func TestNewConfirmation(t *testing.T) {
	...
	model := confirmation.New(title, message, executionID, width)

	assert.Equal(t, title, model.GetTitle())
	assert.Equal(t, message, model.GetMessage())
	assert.Equal(t, executionID, model.GetExecutionID())
	assert.Equal(t, 0, model.GetSelectedIndex()) // Should default to "Yes"
	// Width is internal to the component
}

func TestConfirmationComponent_Init(t *testing.T) {
	model := confirmation.New("Bash Command", "ls -la", "exec-1", 80)
	cmd := model.Init()
	assert.Nil(t, cmd, "Init should return nil command")
}

func TestConfirmationComponent_Navigation(t *testing.T) {
	model := confirmation.New("Test Tool", "test command", "exec-1", 80)
...
func TestConfirmationComponent_DirectSelection(t *testing.T) {
	model := confirmation.New("Test Tool", "test command", "exec-123", 80)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			msg := cmd()
			responseMsg, ok := msg.(confirmation.ResponseMsg)
			require.True(t, ok, "Should return confirmation.ResponseMsg")
...
func TestConfirmationComponent_EnterConfirmsSelection(t *testing.T) {
	model := confirmation.New("Test Tool", "test command", "exec-456", 80)
...
func TestConfirmationComponent_WidthHandling(t *testing.T) {
	model := confirmation.New("Test Tool", "test cmd", "exec-1", 20) // Very narrow width
	view := model.View()
...
func TestConfirmationComponent_MessageHandling(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			model := confirmation.New("Test Tool", tc.message, "exec-1", 80)
			view := model.View()
