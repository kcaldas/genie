...
// turnState drives one chat turn against the DeepSeek API for the
...
// Step runs one blocking model request. DeepSeek's streaming API is
...
func (t *turnState) Step(ctx context.Context, _ func(*ai.StreamChunk)) (llmshared.StepOutcome, error) {
	...
	// toChatMessage drops reasoning_content: DeepSeek rejects it in
	// input messages.
...
// step sees the results. DeepSeek accepts no binary input, so media
// payloads stay as textual descriptions.
func (t *turnState) AddToolResults(_ context.Context, results []llmshared.PreparedToolResult) error {
	for _, result := range results {
		encoded := llmshared.EncodeToolResult(result, supportsNoBlobs)
...
// supportsNoBlobs marks every binary payload as unsupported: the
// DeepSeek API is text-only, so blobs are described in text instead.
func supportsNoBlobs(ai.BlobContent) bool { return false }
