...
func TestPromptCaching_WithDependencyInjection(t *testing.T) {
	...
	mockGen := new(MockGen)
	mockGen.On("GenerateContentAttr", mock.Anything, mock.Anything, true, mock.Anything).Return("Generated response", nil)
	...
	result, err := executor.Execute(ctx, promptName, true)
	...
	nonexistentResult, err := executor.Execute(ctx, "nonexistent", true)
	assert.Error(t, err)
