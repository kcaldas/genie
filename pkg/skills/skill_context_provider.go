...
func (p *SkillContextPartProvider) GetPart(c context.Context) (ctx.ContextPart, error) {
	...
	contentBuilder.WriteString(fmt.Sprintf("When the skill instructions below reference `<SKILL_DIRECTORY>` in bash commands, replace it with: `%s`\n\n", activeSkill.BaseDir))
	contentBuilder.WriteString("**Example command substitution:**\n")
	contentBuilder.WriteString("- Instruction says: `python3 <SKILL_DIRECTORY>/scripts/generate.py input.json`\n")
	contentBuilder.WriteString(fmt.Sprintf("- You execute: `python3 %s/scripts/generate.py input.json`\n", activeSkill.BaseDir))
	contentBuilder.WriteString("\n")
